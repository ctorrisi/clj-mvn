...
module Fluent::Plugin
  class ForwardInput < Input
    Fluent::Plugin.register_input('forwarder', self)
    ...
    def configure(conf)
      ...
      @metrics = {
        bytes_counter: register_metric(::Prometheus::Client::Counter.new(
                                         :forwarder_input_read_bytes_count,
                                         'The number of log bytes read'
                                       )),
      }
    end
    ...
    def read_messages(conn, &block)
      ...
      conn.data do |data|
        ...
        @metrics[:bytes_counter].increment(metric_labels, data.bytesize)
